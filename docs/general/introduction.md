...
We've found that for many applications treating single pages (e.g. the LoginPage, the SearchForm, etc.) as containers and their small parts (e.g. the Login form, the Navigation bar) as components works well, but there are no rigid rules. **Bend the architecture to the needs of your app, nothing is set in stone!**
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3000](https://localhost:3000), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<SearchForm />`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/SearchForm/index.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
...
Check out [`SearchForm/saga.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/SearchForm/saga.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
...
_An update has occurred!_ `mapStateToProps()` will be triggered. `<SearchForm />` will receive the new data and rerender.
