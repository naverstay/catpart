...
export function OrderForm({
  ...
  const contactSubmit = e => {
    ...
    const products = store.map(s => {
      ...
      const price = priceFormatter(parseFloat(s.pricebreaks[priceIndex].price).toFixedCustom(currency.precision), currency.precision);
  ...
  return (
    <div className={`form-order${delivery ? " __delivery" : ""}`}>
      <form ref={formRef} className="form-content" onSubmit={contactSubmit}>
        ...
        <FormInput clear textarea placeholder="Комментарий" name="order-comment" error={null} className="__lg"
                   inputRef={commentInput} />

        {delivery && (
          <>
            <SlideDown>{totalCart > 20000 ?
              <p className={"form-free_shipping"}>Сумма вашего заказа больше 20&nbsp;000 рублей. Для вас доставка за наш
                счёт.</p> : null}</SlideDown>
        ...
        <div className="form-control">
          <Ripples className={`__w-100p btn __blue __lg${validForm ? "" : " __disabled"}${busy ? " __loader" : ""}`}
                   during={1000}>
